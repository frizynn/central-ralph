...
test('Bio section has "About" title', () => {
    assertContains(htmlContent, 'About', 'Bio section has "About" title');
});
...
test('Has SVG social icons', () => {
    assertContains(htmlContent, '<svg', 'Has SVG icons');
    assertContains(htmlContent, 'social-icon', 'Has social icon class');
});
...
test('Has bio section styles', () => {
    assertContains(cssContent, '.bio-section', 'Has bio section styles');
});
...
test('Bio content has responsive layout', () => {
    assert(cssContent.includes('@media (min-width: 768px)') && 
...
test('Avatar placeholder is circular', () => {
    ...
    assertContains(cssContent, 'border-radius: 50%', 'Avatar is circular');
});
...
test('Social icon styling exists', () => {
    assertContains(cssContent, '.social-icon', 'Has social icon styles');
    assertContains(cssContent, 'width: 20px', 'Icon has defined width');
    assertContains(cssContent, 'height: 20px', 'Icon has defined height');
});
...
test('Has form validation functions', () => {
    assertContains(jsContent, 'validateForm', 'Has form validation function');
    assertContains(jsContent, 'isValidEmail', 'Has email validation function');
});
...
test('Bio section has proper heading hierarchy', () => {
    const bioTitleMatch = htmlContent.match(/<section[^>]*id="bio"[^>]*>[\s\S]*?<h2[^>]*>About<\/h2>/);
